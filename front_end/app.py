...
class App(Tk):
    ...
    def detect(self):
        ...
        if directory:
            ...
            def run():
                ...
                if len(self.graphs) == 1: self.after(1000,lambda:[self.show(file_name, self.graphs[0][1], graph),loader.destroy()])
    ...
    # Il metodo visualizza a schermo i risultati del rilevamento.
    #
    # La funzione richiede come parametri:
    #   - un'istanza della classe App;
    #   - il nome del file contenente il grafo;
    #   - il grafo caricato da file (prima del rilevamento);
    #   - il grafo ottenuto dopo il rilevamento.



    def show(self, file_name, graph1, graph2):

        # Crea una nuova figura in cui visualizzare i risultati

        fig = figure(file_name,figsize=(12,7))
        fig.canvas.mpl_connect('close_event', lambda x: exit())

        # Aggiunge due assi alla figura

        ax1,ax2 = fig.subplots(2,1)

        # Aggiunge il primo grafo alla figura

        Plot.draw(graph1, ax1, "Before loop detection")

        # Aggiunge il secodo grafo alla figura

        Plot.draw(graph2, ax2, "After loop detection")
