...
class IgGraph(DiGraph):
    ...
    def detect(self, pattern_type, unpack=False, patterns=[], delta=0):
        ...
        event_log.detect(pattern_type,delta)
    ...
    def addPath(self, labels, predecessors=[], unpack=True):
        ...
        for label in labels:
            ...
            match label:
                ...
                case x if isinstance(x,frozenset): predecessors = [self.addPath(path,predecessors,unpack) for path in (label if unpack else frozenset(list(label)))]
                ...
                case _:
                    ...
                    if isinstance(label,Loop): self.add_edge(curr,curr)
